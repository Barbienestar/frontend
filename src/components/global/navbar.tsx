...
const Navbar: React.FC<NavbarProps> = ({ variant = "default", activePath }) => {
  const config = {
    ...
    default: {
      bg: "bg-white",
      text: "text-black text-lg font-bold",
      subtext: "text-m text-yellow-700",
      title: "DECISION 360",
  ...
  return (
    <nav className={`${current.bg} px-6 py-3 shadow-md`}>
      <div className="relative flex items-center">
        ...
        <div className={`${current.links.length > 0 ? "" : "ml-auto"} ml-auto flex items-center`}>
          {variant === "default" && (
            <button className="p-1 rounded-full hover:bg-gray-100">
              <img src={perfilPhoto} alt="Perfil" className="w-8 h-8 rounded-full" />
            </button>
          )}
