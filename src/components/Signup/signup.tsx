...
const signupSchema = Yup.object().shape({
    name: Yup.string().max(128, "Máximo 128 caracteres").required("El nombre es obligatorio"),
    lastName1: Yup.string().max(64, "Máximo 64 caracteres").required("El apellido paterno es obligatorio"),
    lastName2: Yup.string().max(64, "Máximo 64 caracteres"),
    email: Yup.string().email("Formato inválido").max(255, "Máximo 255 caracteres").required("El correo es obligatorio"),
    password: Yup.string()
        .min(6, "Mínimo 6 caracteres")
        .matches(/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/, "Debe incluir Mayús, minús y número")
...
const SignUp = () => {
    ...
    useEffect(() => {
        getAllStates().then(data => {
            setStates(data.map(item => ({ value: item.id.toString(), label: item.name.toUpperCase() })));
    ...
    useEffect(() => {
        if (formik.values.stateId !== "") {
            getCitiesByState(Number(formik.values.stateId)).then(data => {
                setCities(data.map(item => ({ value: item.id.toString(), label: item.name.toUpperCase() })));
            });
        }
